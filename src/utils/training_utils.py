...
def mp_run_on_node(config, checkpoint_path, eval_only=False, worker=None):
...
def find_gpus():
    if torch.cuda.is_available():
        ...
        warnings.warn("No GPU detected, using CPU instead.")
        ngpus_per_node = 0
...
def mp_run_x(*args, **kwargs):
    ...
    ngpus_per_node = find_gpus()
    if ngpus_per_node == 0:
        mp.spawn(worker, nprocs=1, args=(ngpus_per_node, *args, *kwargs))
    else:
